...
const handlePurchase = async (planName: string) => {
  try {
    ...
    const { data } = await api.post('/api/user/purchase-credits', {
      planId: planName   // ✅ now correct value
    });
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-3 flex-wrap gap-4'>
                        {plans.map((plan, idx) => (
                            <div key={idx} className="p-6 bg-black/20 ring ring-indigo-950 mx-auto w-full max-w-sm rounded-lg text-white shadow-lg hover:ring-indigo-500 transition-all duration-400">
                                ...
                                <button onClick={() => handlePurchase(plan.name)} className="w-full py-2 px-4 bg-indigo-500 hover:bg-indigo-600 active:scale-95 text-sm rounded-md transition-all">
                                    Buy Now
                                </button>
                            </div>
                        ))}
